...
label_1 = 'residual'
label_2 = 'primary'
label_1_comps = [1, 8, 16]
label_2_comps = [6, 18]

diffexp_folder = output_folder + f'/diffexp/{label_1}_vs_{label_2}/'
os.makedirs(diffexp_folder, exist_ok=True)

# residual
vals_1 = np.zeros(len(adata.obsm['chr_aa']))
for i in label_1_comps:
    vals_1 += adata.obsm['chr_aa'][:, i]
positive = vals_1 > threshold
print(np.sum(positive))
adata.obs[label_1] = positive.astype(str)
if show_plot:
    spatial_plot(adata, 2, 3, label_1, share_range=False)
    plt.show()

# primary
vals_2 = np.zeros(len(adata.obsm['chr_aa']))
for i in label_2_comps:
    vals_2 += adata.obsm['chr_aa'][:, i]
positive = vals_2 > threshold
print(np.sum(positive))
adata.obs[label_2] = positive.astype(str)
if show_plot:
    spatial_plot(adata, 2, 3, label_2, share_range=False)
    plt.show()
...
cats = make_obs_categories(adata, [label_1, label_2])
...
if show_plot:
    dc.plot_filter_by_expr(pdata, group='diffexp', min_count=300, min_total_count=200)
    plt.show()

#%%
genes = dc.filter_by_expr(pdata, group='diffexp', min_count=300, min_total_count=200)
...
dds.deseq2()
# statistics
stat_res = DeseqStats(dds, contrast=['diffexp', label_2, label_1])
print(stat_res.design_matrix[f'diffexp_{label_1}_vs_{label_2}'])

#%%

flip = True  # if the printed 0-1 values are reveresed set it to True
if flip:
    # invert the compartments in the design matrix, seems like the 'contrast' method doesn't work as intended
    stat_res.design_matrix[f'diffexp_{label_1}_vs_{label_2}'] = (
        np.abs(stat_res.design_matrix[f'diffexp_{label_1}_vs_{label_2}'] - 1))

stat_res.summary()
# lfc shrinking
stat_res.lfc_shrink(coeff=f'diffexp_{label_1}_vs_{label_2}')
results_df = stat_res.results_df

if flip:
    # stat column is bugged?
...
if show_plot:
    ...
    fig.savefig(f'figs/manuscript/fig4/{label_1}_{label_2}_volcano2.svg')
    plt.show()

results_df['-log10padj'] = np.log10(results_df['padj']) * -1
results_df.to_csv(diffexp_folder + 'results.csv')

#%%
# functional analysis

results_df = pd.read_csv(diffexp_folder + 'results.csv', index_col=0)

mat = results_df[['stat']].T.rename(index={'stat': label_1})

# transcription factor
tri = pd.read_csv('data/decoupler/tri_human_tfs.csv', index_col=0)
tf_acts, tf_pvals = dc.run_ulm(mat=mat, net=tri)
fig = dc.plot_barplot(tf_acts, label_1, top=25, vertical=True, figsize=(5, 5), return_fig=True)
...
fig.savefig(f'figs/manuscript/fig4/{label_1}_{label_2}_tf.svg')
plt.show()


# tf scatters
logFCs = results_df[['log2FoldChange']].T.rename(index={'log2FoldChange': label_1})
pvals = results_df[['padj']].T.rename(index={'padj': label_1})

top_tfs = np.abs(tf_acts.T)
top_tfs = top_tfs.sort_values(by=label_1, ascending=False)
top_tfs = top_tfs.iloc[:25, 0]

os.makedirs(diffexp_folder + f'/tf_scatter/', exist_ok=True)
for c in top_tfs.index:
    fig, ax = plt.subplots(1, 1, figsize=(5, 5))
    dc.plot_volcano(logFCs, pvals, label_1, name=c, net=tri, top=10, sign_thr=0.05, lFCs_thr=0.5,
                    ax=ax)
    ax.grid(axis='both', linestyle='-', linewidth='0.5', color='grey')
    ax.set_axisbelow(True)
    ax.set_title(c)
    fig.savefig(diffexp_folder + f'/tf_scatter/{c}.png')
    plt.clf()
    plt.close()
...
plt.rcParams['svg.fonttype'] = 'none'
fig, ax = plt.subplots(1, 1, figsize=(5, 5))
fig = dc.plot_barplot(pathway_acts, label_1, top=25, vertical=False, return_fig=True, figsize=(5, 5))
fig.axes[0].spines['top'].set_visible(False)
fig.axes[0].spines['right'].set_visible(False)
...
cbar.set_frame_on(False)
plt.tight_layout()
fig.savefig(f'figs/manuscript/fig4/{label_1}_{label_2}_progeny.svg')
plt.show()


os.makedirs(diffexp_folder + f'/progeny_scatter/', exist_ok=True)
for c in pathway_acts.columns:
    fig, ax = plt.subplots(1, 1, figsize=(5, 5))
    fig = dc.plot_targets(results_df, stat='stat', source_name=c, net=progeny, top=15, return_fig=True)
    fig.savefig(diffexp_folder + f'/progeny_scatter/{c}.png')
    plt.clf()
    plt.close()

# hallmarks
gene_sets_df = pd.read_csv(f'data/decoupler/hallmark_human.csv', index_col=0)
...
cbar.set_frame_on(False)
plt.tight_layout()
fig.savefig(f'figs/manuscript/fig4/{label_1}_{label_2}_upreg2.svg')
plt.show()
...
for c in enr_pvals['Term'][:5]:
    ...
    fig.savefig(f'figs/manuscript/fig4/UP_{c}2.svg')
    plt.show()
...
cbar.set_frame_on(False)
plt.tight_layout()
fig.savefig(f'figs/manuscript/fig4/{label_1}_{label_2}_downreg2.svg')
plt.show()
...
for c in enr_pvals['Term'][:5]:
    ...
    fig, _ = dc.plot_running_score(results_df, stat='stat', net=gene_sets_df, source='geneset', target='genesymbol',
                                   set_name=c, return_fig=True)
    fig.savefig(f'figs/manuscript/fig4/DOWN_{c}2.svg')
    plt.show()
...
fig, _ = dc.plot_running_score(results_df, stat='stat', net=gene_sets_df, source='geneset', target='genesymbol',
                               set_name='HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION', return_fig=True)
plt.show()

spatial_plot(adata, 2, 3, 'HOOK1', share_range=False)
plt.show()

sc.pl.violin(adata, 'CLIP3', groupby='condition', use_raw=False)

marker_genes_dict ={
  'upreg': list(results_df.sort_values(by='stat', ascending=False).index[:10]),
  'downreg': list(results_df.sort_values(by='stat', ascending=True).index[:10])
}
sc.pl.stacked_violin(adata, marker_genes_dict, groupby='condition', swap_axes=False, dendrogram=True,
                 cmap='Blues', use_raw=False)
