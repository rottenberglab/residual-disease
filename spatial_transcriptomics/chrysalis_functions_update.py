...
from chrysalis.utils import (generate_random_colors, black_to_color, get_rgb_from_colormap,
                             mip_colors, color_to_color, get_compartment_df, get_hexcodes)


# Updated functions to streamline visualizations using the Chrysalis package
...
def plot_heatmap(adata: AnnData, figsize: Tuple[int, int]=(5, 7), reorder_comps: bool=False, hexcodes: List[str]=None,
    ...
    # SVG weights for each compartment
    df = pd.DataFrame(data=adata.uns['chr_aa']['loadings'], columns=adata.uns['chr_pca']['features']).T
    if scaling:
        df = df.apply(lambda x: (x-x.min())/ (x.max()-x.min()), axis=0).T
        df = df
    dim = df.shape[0]
...
def plot_weights(adata: AnnData, hexcodes: List[str]=None, seed: int=None, compartments: List[int]=None, ncols: int=4,
                 w: float=1.0, h: float=1.0, top_genes=20):
    """
    Plot 20 top genes for each tissue compartment.
